...
int main(){
	...
	// 阻止了这种隐式转换的发生
	// demo 代码见 [08_explicit.cpp](https://github.com/abcdabcd3899/Dive-Into-Modern-CPP/blob/main/08_explicit.cpp)
...
```
* item 4: shared_ptr 循环引用导致内存泄漏

* item 5: 不能返回 std::shared_ptr(this),这会导致抢夺所有权的问题, 应该使用 std::enable_shared_from_this<T>;

* 调用 shared_from_this 方法的对象必须是 shared_ptr,否则会有运行时错误
...
namespace test_shared_from_this{
	int main(){
		// Widget w ;
		// std::shared_ptr<Widget> ptr1 = w.getWidget();  // 错误

		// Widget* w = new Widget();
		// std::shared_ptr<Widget> ptr2 = w->getWidget();  // 错误

		std::shared_ptr<Widget> w = std::make_shared<Widget>();
		std::shared_ptr<Widget> ptr2 = w->getWidget();  // 正确
		return 0;
	}
}

int main(){
	test_shared_from_this::main();
	return 0;
}
```

从上面代码我们看出,实际上我们应该阻止那两种错误的构造方法, 如果不阻止,总会有人构造出栈或者普通对象.
怎么解决呢? 将普通构造函数设为私有,然后使用工厂方法构造.

```c++
struct Widget1: public std::enable_shared_from_this<Widget1>{
	std::shared_ptr<Widget1> getWidget(){
		return shared_from_this();
	}
	~Widget1(){
		std::cout << "----~Widget()----" << std::endl;
	}
	static std::shared_ptr<Widget1> create(){
		return std::shared_ptr<Widget1>(new Widget1());
		// return std::make_shared<Widget1>(); // 这个编译通不过, why?
	}
private:
	Widget1()=default;
};
namespace test_shared_from_this_factory{
	int main(){
		// Widget1 w ;  // 直接报错,这里调用了 private 的构造函数
		// std::shared_ptr<Widget1> ptr1 = w.getWidget();  // 错误

		// Widget1* w = new Widget1(); // 直接报错,这里调用了 private 的构造函数
		// std::shared_ptr<Widget> ptr2 = w->getWidget();  // 错误

		std::shared_ptr<Widget1> w = Widget1::create();
		std::shared_ptr<Widget1> ptr2 = w->getWidget();  // 正确
		return 0;
	}
}

int main(){
	test_shared_from_this_factory::main();
	return 0;
}
```
#### weak_ptr

weak_ptr 是 shared_ptr 的特殊实现,内存模型和 shared_ptr 完全一致.
